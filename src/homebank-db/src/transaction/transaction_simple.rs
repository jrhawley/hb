...
pub struct SimpleTransaction {
    ...
    /// The memo of the parent [`Transaction`][crate::transaction::transaction::Transaction].
    /// This will duplicate data, but this impacts the code base much less
    /// than using pointers and introducing lifetimes everywhere.
    memo: Option<String>,
}
...
impl SimpleTransaction {
    ...
    /// Retrieve the amount of the [`Transaction`][crate::transaction::transaction::Transaction].
    pub fn amount(&self) -> &f32 {
        &self.amount
    }

    /// Retrieve the mutable amount of the [`Transaction`][crate::transaction::transaction::Transaction].
    pub fn mut_amount(&mut self) -> &mut f32 {
        &mut self.amount
    }

    /// Retrieve the memo for the [`Transaction`][crate::transaction::transaction::Transaction].
    pub fn memo(&self) -> &Option<String> {
        &self.memo
    }

    /// Retrieve the mutable memo for the [`Transaction`][crate::transaction::transaction::Transaction].
